...
# endif
    ...
    maxtimestep = 0.5_dp**(N_DIMS-1)*CFL/REAL(N,dp) * dtmin
...
subroutine timestep_cb(quad)
    ...
    use equations_mod, only: zlambdaMax
...
# endif
    ...
    real(dp) :: lmaxF !(N_NODES,N_NODES,N_NODES)
    real(dp) :: lmaxG !(N_NODES,N_NODES,N_NODES)
    real(dp) :: lmaxH !(N_NODES,N_NODES,N_NODES)
    real(dp) :: dt

    lmaxF = maxval(xlambdaMax(quad%pld%hydro%state))
    lmaxG = maxval(ylambdaMax(quad%pld%hydro%state))
    lmaxH = maxval(zlambdaMax(quad%pld%hydro%state))

# if PP_EQN_MHD
    associate(u => quad%pld%hydro%state)
    lmax = max(lmaxF,lmaxG,lmaxH)
    vmax = max(maxval(abs(u(:,:,:,MOMX_VAR)/u(:,:,:,DENS_VAR))),maxval(abs(u(:,:,:,MOMY_VAR)/u(:,:,:,DENS_VAR))),maxval(abs(u(:,:,:,MOMZ_VAR)/u(:,:,:,DENS_VAR))))
    end associate
# endif
    ...
    dt = min(dt,min(quad%delta(1)/lmaxF,quad%delta(2)/lmaxG,quad%delta(3)/lmaxH))
