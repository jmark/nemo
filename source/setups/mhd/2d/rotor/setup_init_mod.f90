...
subroutine fill(quad)
    ...
    real(dp) :: transition(N_NODES,N_NODES)

    coords = quad_get_coords(quad)
    radius = SQRT(SUM(coords**2,dim=N_DIMS+1))

    transition = (radius_outer-radius)/(radius_outer-radius_inner)
    ...
    where (radius <= radius_inner)

        dens =  10.0_dp
        velx = -20.0_dp * coords(:,:,Y_DIR)
        vely =  20.0_dp * coords(:,:,X_DIR)

    else where (radius < radius_outer)

        dens =  dens0 + 9.0_dp*transition
        velx = -20.0_dp * coords(:,:,Y_DIR) * transition
        vely =  20.0_dp * coords(:,:,X_DIR) * transition
...
function setup_init_probe_refine(quad) result(ok)
    ...
    if (any(abs(SQRT(SUM((verts)**2,dim=1))) < 0.15_dp)) then
        ok = .true.
    end if
