...
subroutine fill(quad)
    ...
    velx = -(1+eps*sin(2.0_dp*pi*coords(:,:,:,Z_DIR)))*sin(2.0_dp*pi*coords(:,:,:,Y_DIR))
    vely =  (1+eps*sin(2.0_dp*pi*coords(:,:,:,Z_DIR)))*sin(2.0_dp*pi*coords(:,:,:,X_DIR))
    velz =     eps*sin(2.0_dp*pi*coords(:,:,:,Z_DIR))

    magx = -sin(2.0_dp*pi*coords(:,:,:,Y_DIR))/kappa
    magy =  sin(4.0_dp*pi*coords(:,:,:,X_DIR))/kappa
    magz = 0.0_dp/kappa
