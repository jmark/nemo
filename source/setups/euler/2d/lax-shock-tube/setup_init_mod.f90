...
subroutine fill(quad)
    ...
    logical :: template(N_NODES,N_NODES)
    ...
    ! template = radius <= pivot
    template = center(1) <= pivot

    dens = merge(densL,densR,template)
    velx = merge(velxL,velxR,template)
    vely = merge(velyL,velyR,template)
    pres = merge(presL,presR,template)
