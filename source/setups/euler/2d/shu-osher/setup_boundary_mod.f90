...
subroutine setup_side_boundary(side,inner,outer)
    ...
    select case(side%boundary)
        ...
        case (NOR)
            ...
            outer(:,MOMY_VAR) = densL*vely0
            outer(:,ENER_VAR) = presL/(kappa-1.0_dp) + 0.5_dp*densL*(velxL**2+vely0**2)

        case (SOU)
            outer(:,DENS_VAR) = densR 
            outer(:,MOMX_VAR) = densR*velxR
            outer(:,MOMY_VAR) = densR*vely0
            outer(:,ENER_VAR) = presR/(kappa-1.0_dp) + 0.5_dp*densR*(velxR**2+vely0**2)
