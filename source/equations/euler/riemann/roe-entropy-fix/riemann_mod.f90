...
pure function xriemann(U_L,U_R) result(F)
    ...
    real(dp) :: RoeCStab
    real(dp), parameter :: RoePhi = 5.0_dp
    ...
    !! anti-carbuncle fix
    RoeCStab = min(RoePhi*abs(RoeVel(X_DIR)),RoeC)
    lambda  = (/ RoeVel(X_DIR)-RoeCStab, RoeVel(X_DIR), RoeVel(X_DIR), RoeVel(X_DIR), RoeVel(X_DIR)+RoeCStab/)
